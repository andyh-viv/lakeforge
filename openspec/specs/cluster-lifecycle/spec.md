...
`RUNNING` while its driver is alive, `TERMINATED` once the driver has exited, and
`SUSPENDED` when it has been idle-suspended — on every supported platform, not
only the one with `/proc`. Exited processes this backend spawned are reaped, so a
dead driver can never read as live and no handle is retained past its process.
